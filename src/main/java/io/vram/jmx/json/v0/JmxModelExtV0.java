...
public class JmxModelExtV0 extends JmxModelExt<JmxModelExtV0> {
	...
	public BakedModel buildModel(BakedOverrides modelOverrideList, boolean hasDepth, TextureAtlasSprite particleSprite, ModelState bakeProps, ResourceLocation modelId, BlockModel me, Function<Material, TextureAtlasSprite> textureGetter) {
		...
		final MaterialFinder finder = JmxRendererAccess.getRenderer().materialFinder();
		...
		for (final BlockElement element : me.getElements()) {
			for (final Direction face : element.faces.keySet()) {
				...
				final FaceExtDataV0 extData = ((JmxExtension<FaceExtDataV0>) (Object) elementFace).jmx_ext();
