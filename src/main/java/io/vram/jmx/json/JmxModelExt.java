...
public abstract class JmxModelExt<Self extends JmxModelExt<Self>> {
  // These appear to be accessed by multiple threads but each thread has different result,
  // through testing this worked tho it doesn't feel like its exactly 'fine'.
	public static ThreadLocal<JmxModelExt<?>> TRANSFER = new ThreadLocal<>();
	public static ThreadLocal<Integer> VERSION = new ThreadLocal<>();
	...
	public abstract BakedModel buildModel(BakedOverrides modelOverrideList, boolean hasDepth, TextureAtlasSprite particleSprite, ModelState bakeProps, ResourceLocation modelId, BlockModel me, Function<Material, TextureAtlasSprite> textureGetter);
